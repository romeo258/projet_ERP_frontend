...
import { AccountType, CustomHttpResponse, Page, Profile, UserState } from '../interface/appstates';
...
export class UserService {
  ...
  updateRolesId$ = (userId: number, roleName: string) =>
    <Observable<CustomHttpResponse<Profile>>>(
      this.http
        .patch<CustomHttpResponse<Profile>>(
          `${this.server}/user/update/role/${userId}/${roleName}`,
          {}
        )
        .pipe(tap(console.log), catchError(this.handleError))
    );
  ...
  updateAccountSettingsById$ = (userId: number, settings: {
    enabled: boolean;
    notLocked: boolean;
  }) =>
    <Observable<CustomHttpResponse<Profile>>>(
      this.http
        .patch<CustomHttpResponse<Profile>>(
          `${this.server}/user/update/${userId}/settings`,
          settings
        )
        .pipe(tap(console.log), catchError(this.handleError))
    );
  ...
  searchUsers$ = (firstName: string = '', page: number = 0) =>
    <Observable<CustomHttpResponse<Page<User> & User>>>(
      this.http
        .get<CustomHttpResponse<Page<User> & User>>(
  ...
  userDetail$ = (userId: number) =>
    <Observable<CustomHttpResponse<UserState>>>(
      this.http
        .get<CustomHttpResponse<UserState>>(`${this.server}/user/get/${userId}`)
        .pipe(tap(console.log), catchError(this.handleError))
    );
