...
export class CategoriesComponent implements OnInit {
  ...
  categoriesState$: Observable<State<CustomHttpResponse<Page<Category> & User>>>;
  ...
  ngOnInit(): void {
    this.categoriesState$ = this.categoryService.searchCategories$()
      .pipe(
        map(response => {
  ...
  searchCategories(searchForm: NgForm): void {
    this.currentPageSubject.next(0);
    this.categoriesState$ = this.categoryService.searchCategories$(searchForm.value.name)
      .pipe(
        map(response => {
          // console.log(response);
          this.dataSubject.next(response);
  ...
  selectCustomer(category: Category): void {
    this.router.navigate([`/categories/${category.id}`]);
  }

  goToPage(pageNumber?: number, name?: string): void {
    this.categoriesState$ = this.categoryService.searchCategories$(name, pageNumber)
      .pipe(
        map(response => {
          // console.log(response);
          this.dataSubject.next(response);
